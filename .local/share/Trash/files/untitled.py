...
def s3_source(spark, mode, dataset_definition):
    ...
    try:
        if content_type == "CSV":
            return default_spark(spark.read.csv(path=path, header=has_header, escape='"', quote='"'))
...
op_1_output = s3_source(spark=spark, mode=mode, **{'dataset_definition': {'__typename': 'S3CreateDatasetDefinitionOutput', 'datasetSourceType': 'S3', 'name': 'crx.csv', 'description': None, 's3ExecutionContext': {'__typename': 'S3ExecutionContext', 's3Uri': 's3://sagemaker-us-west-2-367158743199/sagemaker-toturial/data/crx.csv', 's3ContentType': 'csv', 's3HasHeader': True}}})
op_2_output = infer_and_cast_type(op_1_output['default'], spark=spark, **{})
op_3_output = manage_columns(op_2_output['default'], spark=spark, **{'operator': 'Move column', 'move_column_parameters': {'move_type': 'Move to start', 'move_to_start_parameters': {'column_to_move': 'Approved'}}, 'drop_column_parameters': {}})
op_5_output = handle_missing(op_3_output['default'], spark=spark, **{'operator': 'Drop missing', 'drop_missing_parameters': {'dimension': 'Drop Rows', 'drop_rows_parameters': {'input_column': 'Age'}}, 'impute_parameters': {'column_type': 'Numeric', 'numeric_parameters': {'strategy': 'Approximate Median'}}})
op_6_output = handle_missing(op_5_output['default'], spark=spark, **{'operator': 'Drop missing', 'drop_missing_parameters': {'dimension': 'Drop Rows', 'drop_rows_parameters': {'input_column': 'ZipCode'}}, 'impute_parameters': {'column_type': 'Numeric', 'numeric_parameters': {'strategy': 'Approximate Median'}}})

#  Glossary: variable name to node_id
#
#  op_1_output: ed33deed-f061-4b57-b9ed-cb8b68f7c78c
#  op_2_output: 54127d4f-cc3c-427e-9897-b26aba5e6448
#  op_3_output: 29c80819-db46-49b6-8331-ae4e151af165
#  op_5_output: ae08916d-0382-462e-ba07-26174ae27b5c
#  op_6_output: d5e69256-194c-4f2c-86e9-a80c2a18b79a